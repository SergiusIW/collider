...
final class Field {
	...
	Iterable<HitBox> iterator(IntBox region, IntArray groups, int testId) {
...
//		return ((x & 0xFFFF0000L) << 32) | ((y & 0xFFFFFFFFL) << 16) | (x & 0xFFFFL);
	...
	private class HitBoxIter implements Iterator<HitBox>, Iterable<HitBox> {
		...
		private void init(IntBox region, IntArray groups, int testId) {
			clear();
		...
		private void initCellIter() {
			long key = getKey(boxIter.getX(), boxIter.getY(), groups.get(groupIndex));
